...
export async function getAthleteProfile() {
  const { data, error } = await supabase.from("athlete_profile").select("*").limit(1).single();
  if (error) { console.error("Error fetching profile:", error); return null; }
  return data;
}

export async function updateAthleteProfile(updates) {
  const { data: current } = await supabase.from("athlete_profile").select("id").limit(1).single();
  if (!current) return null;
  const { data, error } = await supabase.from("athlete_profile").update(updates).eq("id", current.id).select().single();
...
export async function getRaces() {
  ...
  if (error) { console.error("Error fetching races:", error); return []; }
  return data;
}
...
export async function getEnergyLog(date) {
  const { data, error } = await supabase.from("energy_logs").select("*").eq("date", date).single();
...
export async function getEnergyLogsRange(startDate, endDate) {
  const { data, error } = await supabase
    ...
    .lte("date", endDate)
    .order("date");
  if (error) { console.error("Error fetching energy range:", error); return []; }
  return data;
}

export async function upsertEnergyLog(log) {
  const { data, error } = await supabase.from("energy_logs").upsert(log, { onConflict: "date" }).select().single();
...
export async function getLatestBurnout() {
  const { data, error } = await supabase.from("burnout_scores").select("*").order("date", { ascending: false }).limit(1).single();
  if (error && error.code !== "PGRST116") console.error("Error fetching burnout:", error);
  return data;
}

export async function upsertBurnoutScore(score) {
  const { data, error } = await supabase.from("burnout_scores").upsert(score, { onConflict: "date" }).select().single();
...
export async function getSupplementLog(date) {
  const { data, error } = await supabase.from("supplement_logs").select("*").eq("date", date).single();
  if (error && error.code !== "PGRST116") console.error("Error fetching supplements:", error);
  return data;
}

export async function upsertSupplementLog(log) {
  const { data, error } = await supabase.from("supplement_logs").upsert(log, { onConflict: "date" }).select().single();
...
export async function addGymSession(session) {
  const { data, error } = await supabase.from("gym_sessions").insert(session).select().single();
