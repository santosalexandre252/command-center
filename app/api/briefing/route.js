...
export async function GET() {
  ...
  const [wellness30, wellness3, activitiesWeek, plannedToday, calendarText] = await Promise.all([
    ...
    ICAL_URL ? fetch(ICAL_URL, { next: { revalidate: 300 } }).then((r) => r.ok ? r.text() : null).catch(() => null) : null,
  ]);
  ...
  const briefingData = {
    date: today,
    wellness: { ctl, atl, tsb, hrv, restingHR, weight, sleepLast },
    hrvAnalysis: { hrvLast3, avg30HRV },
    recentSleep,
    calendar: {
      events: todayEvents.map((e) => ({
        title: e.title,
        start: e.start?.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false }),
        end: e.end?.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false }),
      })),
...
4. End with one specific tip for the day (recovery, nutrition, or mindset).`;
  ...
  return NextResponse.json({ ...briefingData, aiRecommendation });
}
