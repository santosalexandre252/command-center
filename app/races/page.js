...
import { getAthleteProfile, getRaces } from "../../lib/db";
    ...
    <Link href={`/race/${race.id}`}>
      <div className="bg-surface-850 border border-white/5 rounded-xl p-5 card-hover cursor-pointer">
        <div className="flex items-start justify-between mb-3">
          ...
          <span className={`text-xs px-2 py-1 rounded font-bold ${race.priority === "A" ? "bg-brand-600/20 text-brand-400" : "bg-gray-700/50 text-gray-400"}`}>{race.priority}-Race</span>
        </div>
        <div className="grid grid-cols-4 gap-4 mt-4">
          <div><div className="text-[10px] text-gray-500">Date</div><div className="text-sm font-medium text-gray-200">{formatDate(race.date)}</div></div>
          <div><div className="text-[10px] text-gray-500">Distance</div><div className="text-sm font-medium text-gray-200">{race.distance}km</div></div>
          <div><div className="text-[10px] text-gray-500">Elevation</div><div className="text-sm font-medium text-gray-200">{race.elevation}m D+</div></div>
          <div><div className="text-[10px] text-gray-500">Countdown</div><div className="text-sm font-bold text-white">{days} days</div></div>
        </div>
        <div className="mt-4">
...
export default function RacesPage() {
  ...
  useEffect(() => {
    async function load() {
      const [r, a] = await Promise.all([getRaces(), getAthleteProfile()]);
      setRaces(r);
      setAthlete(a);
      setLoading(false);
    }
  ...
  if (loading) return <div className="p-6 text-gray-500 text-sm">Loading races...</div>;
  ...
  return (
    <div className="p-6 max-w-[1400px]">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="px-4 py-2 bg-brand-600 text-white text-sm rounded-lg hover:bg-brand-700 transition-colors">+ Add Race</button>
      </div>
      <div className="grid grid-cols-2 gap-4">
        {races.map((race) => (
          <RaceCard key={race.id} race={race} athlete={athlete} />
        ))}
      </div>
    </div>
  );
